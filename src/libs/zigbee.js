    ...
    co(function* () {
      ...
      if (status == 0) {
        ...
        if (dev) {
          ...
          for (let i = 0; i < activeEpList.length; i++) {
            const ep = activeEpList[i];
            yield write('ZDO_SIMPLE_DESC_REQ', {
              nwk: nwkAddr,
              ep
            });
          }
    ...
    co(function* () {
      ...
      if (status == 0) {
        ...
        if (app) {
          ...
          let payload;
          switch (deviceId) {
            case config.get('zigbee_appType_lamp'):
              type = 'lamp';
              payload = {
                on: false
              };
              break;
            case config.get('zigbee_appType_gray_lamp'):
              type = 'gray-lamp';
              payload = {
                level: 0
              };
              break;
            case config.get('zigbee_appType_switch'):
              type = 'switch';
              payload = {
                on: false
              };
              break;
            case config.get('zigbee_appType_gray_switch'):
              type = 'gray-switch';
              payload = {
                level: 0
              };
              break;
            case config.get('zigbee_appType_pulse'):
              type = 'pulse';
              payload = {
                transId: 0
              };
              break;
            case config.get('zigbee_appType_illuminance_sensor'):
              type = 'illuminance-sensor';
              payload = {
                level: 0
              };
              break;
            case config.get('zigbee_appType_temperature_sensor'):
    ...
    if (areqParserMap.hasOwnProperty(name)) {
      ...
      if (typeof (parser) === 'function') {
        try {
          ...
          log.error(`${name} 处理器未捕获的错误\n`, err)
        }
      } else {
        log.warn(`areqParser '${name}' is not a function`)
      }
    } else {
