...
const { sceneChooser: log } = require('../utils/log');
...
const getAllSensorResults = co.wrap(function * () {
  ...
  const sensors = yield App
    .find()
    .or([{type: 'illuminance-sensor'}, {type: 'temperature-sensor'}])
    .exec();
...
const getNormalizedSensorResults = co.wrap(function * () {
  ...
  for (let i=0; i<rResults.lenght; i++) {
    const {type, payload} = rResults[i];
    const range = config[`app/${type}/range`];
    if (range) {
      const [min, max] = range;
      const firstProperty = Object.keys(payload)[0];
      const value = new Number(payload[firstProperty]);
      const nValue = (value-min) / (max-min);
      nResults.push({type, payload: Object.assign({}, payload, {[firstProperty]: nValue})});
    } else {
      log.warn(`未找到配置项：app/${type}/range`)
    }
  }
  return nResults;
})
...
const selectSceneId = co.wrap(function * () {
  const { JudgeRuleGroup, App, Device } = models;
  const allGroups = yield JudgeRuleGroup.find().exec();
  for (let i=0; i<allGroups.length; i++) {
    const {scene, rules} = allGroups[i];
    let j=0;
    for (j=0; j<rules.length; j++) {
      log.trace('取出rule\n', rules[i]);
      const {ieee, ep, type, payload} = rules[j];
      const device = yield Device.findOne({ieee}).exec();
      log.trace('找到device\n', device);
      if (device) {
        const app = yield App.findOne({device: device.nwk, endPoint: ep}).exec();
        if (app) {
          log.trace('检测APP\n', app);
          let ruleMatch = false;
          const appPayloadValue = app.payload[Object.keys(app.payload)[0]];
          switch (type) {
            case 'equal': ruleMatch = payload === appPayloadValue; break;
            case 'nequal': ruleMatch = payload !== appPayloadValue; break;
            case 'range': ruleMatch = (payload[0] <= appPayloadValue) && (appPayloadValue <= payload[1]); break;
            case 'nrange': ruleMatch = (payload[0] >= appPayloadValue) || (appPayloadValue >= payload[1]); break;
            case 'gt': ruleMatch = payload <= appPayloadValue; break;
            case 'lt': ruleMatch = payload >= appPayloadValue; break;
            case 'ignore':
            default: ruleMatch = true;
          }
          log.trace('匹配结果', ruleMatch);
          // 如果有未匹配的规则，则跳出rule for循环
          if (!ruleMatch) break;
        } else { throw new Error(`未找到硬件地址为 ${ieee} 的设备`) }
      } else { throw new Error(`未找到硬件地址为 ${ieee} 的设备`) }
    }
...
class Chooser extends EventEmitter {
  ...
  _handleSensorFeedback({nwk, ep, type, payload}) {
    const self = this;
